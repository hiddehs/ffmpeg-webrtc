...
# if [[ $compiler_flavors == "multi" || $compiler_flavors == "win64" ]]; then
  ...
  mingw_w64_x86_64_prefix=/usr
  # /opt/homebrew/Cellar/mingw-w64/11.0.1/toolchain-x86_64/$host_target
...
build_openssl() {
  ...
  # exit 1
  ./Configure no-tests mingw64 no-threads --libdir=$prefix/lib --prefix=$prefix || exit 1
...
# fi
 ...
 build_libopus() {
  ...
  cd opus_git
  autoreconf -fiv
  ./configure --prefix=$prefix --disable-shared --enable-static --disable-doc --disable-extra-programs --disable-stack-protector
  make -j$cpu_count
  make install || exit 1
  cd ..
 }
